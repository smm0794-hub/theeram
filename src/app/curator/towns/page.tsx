...
export default function TownsPage() {
  ...
  async function runTownAgent() {
    ...
    setAgentLog([`Researching ${agentInput}...`, 'Calling agent...'])
    try {
  ...
  async function saveTown() {
    ...
    showToast(isNew ? 'Town added!' : 'Saved')
    await loadTowns()
  ...
  function field(label: string, key: keyof Town, multiline?: boolean) {
    ...
    return (
      <div key={key} style={{ marginBottom: 4 }}>
        ...
        {multiline
          ? <textarea value={(editing[key] as string) ?? ''} onChange={e => setEditing(prev => ({...prev!, [key]: e.target.value}))} rows={4} style={{ ...inp,resize:'vertical' as const }}/>
  ...
  if (mode === 'agent') return (
    <div style={{ minHeight:'100vh',background:C.cream,paddingBottom:80 }}>
      ...
      <div style={{ background:C.cream,borderBottom:`1px solid ${C.cream3}`,padding:'13px 16px',display:'flex',gap:10,alignItems:'center' }}>
        <button onClick={() => { setMode('list'); setAgentResult(null); setAgentLog([]) }} style={{ ...sans,fontSize:12,color:C.muted,background:'none',border:'none',cursor:'pointer' }}>Back</button>
      ...
      <div style={{ padding:'20px 16px' }}>
        <div style={{ background:C.green,padding:'16px',marginBottom:20 }}>
          <p style={{ ...sans,fontSize:13,color:'rgba(255,255,255,.7)',lineHeight:1.7,fontWeight:300,margin:0 }}>Enter a town name. The agent researches it and generates all hero copy, stats, and venue search queries.</p>
        </div>
        ...
        <button onClick={runTownAgent} disabled={agentRunning||!agentInput.trim()} style={{ ...sans,width:'100%',background:agentRunning?C.muted:C.green,color:'white',fontSize:11,fontWeight:700,letterSpacing:'.1em',textTransform:'uppercase' as const,padding:'13px 0',border:'none',cursor:agentRunning?'not-allowed':'pointer',marginBottom:20 }}>
          {agentRunning ? 'Researching...' : 'Generate town content'}
        </button>
        {agentLog.length > 0 && <div style={{ background:C.green2,padding:'12px 14px',marginBottom:20 }}>{agentLog.map((l,i) => <p key={i} style={{ ...mono,fontSize:11,color:i===agentLog.length-1?C.gold:'rgba(255,255,255,.45)',margin:'3px 0' }}>{l}</p>)}</div>}
        ...
        {agentResult && (
          <div style={{ border:`1px solid ${C.cream3}`,background:'white',padding:16,marginBottom:20 }}>
            ...
            <div style={{ ...sans,fontSize:11,color:C.muted,marginBottom:8 }}>{agentResult.district} District</div>
            <div style={{ ...sans,fontSize:12,color:C.muted,lineHeight:1.6,marginBottom:10,fontWeight:300 }}>{agentResult.tagline}</div>
            {agentResult.suggested_search_queries?.length > 0 && <div style={{ marginBottom:12 }}><p style={{ ...sans,fontSize:9,color:C.terra,letterSpacing:'.07em',marginBottom:6 }}>SUGGESTED SEARCHES</p>{agentResult.suggested_search_queries.map((q: string) => <p key={q} style={{ ...sans,fontSize:11,color:C.muted,marginBottom:3 }}>- {q}</p>)}</div>}
            <button onClick={useAgentResult} style={{ ...sans,width:'100%',background:C.green,color:'white',fontSize:11,fontWeight:700,letterSpacing:'.1em',textTransform:'uppercase' as const,padding:'12px 0',border:'none',cursor:'pointer' }}>Review and save</button>
          </div>
        )}
      </div>
    </div>
  ...
  if (mode === 'edit' && editing) return (
    <div style={{ minHeight:'100vh',background:C.cream,paddingBottom:80 }}>
      ...
      <div style={{ background:C.cream,borderBottom:`1px solid ${C.cream3}`,padding:'13px 16px',display:'flex',gap:10,alignItems:'center',position:'sticky',top:0,zIndex:40 }}>
        <button onClick={() => { setMode('list'); setEditing(null) }} style={{ ...sans,fontSize:12,color:C.muted,background:'none',border:'none',cursor:'pointer' }}>Back</button>
      ...
      <div style={{ padding:'20px 16px 0' }}>
        ...
        {field('Hero headline', 'hero_headline', true)}
        {field('Hero subtext', 'hero_subtext', true)}
        {field('Why here heading', 'why_here_heading')}
        {field('Why here text', 'why_here_text', true)}
        {field('Stat 1 value', 'stat_1_value')}
        {field('Stat 1 label', 'stat_1_label')}
        ...
        {field('Hero background color', 'hero_bg_color')}
  ...
  return (
    <div style={{ minHeight:'100vh',background:C.cream,paddingBottom:60 }}>
      <div style={{ background:C.green,padding:'6px 16px',display:'flex',justifyContent:'space-between' }}><span style={{ ...sans,fontSize:10,color:'rgba(255,255,255,.5)' }}>theeram</span></div>
      <div style={{ background:C.cream,borderBottom:`1px solid ${C.cream3}`,padding:'13px 16px',display:'flex',justifyContent:'space-between',alignItems:'center',position:'sticky',top:0,zIndex:40 }}>
        ...
        <div style={{ display:'flex',gap:8 }}>
          <button onClick={() => setMode('agent')} style={{ ...sans,background:'transparent',color:C.terra,fontSize:10,fontWeight:600,padding:'7px 12px',border:`1px solid ${C.terra}`,cursor:'pointer' }}>Agent</button>
        ...
        {towns.map(t => (
          <div key={t.id} style={{ background:'white',border:`1px solid ${C.cream3}`,padding:14 }}>
            <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10 }}>
              ...
              <span style={{ ...sans,fontSize:10,fontWeight:600,padding:'3px 8px',background:t.is_active?'#f0faf4':'#faf0eb',color:t.is_active?'#2D7A4F':C.muted }}>{t.is_active?'Live':'Draft'}</span>
            </div>
